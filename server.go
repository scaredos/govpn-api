...
// Author: Unknown-Industries-LLC \ scaredos (https://github.com/scaredos)
// Written for simple management of SoftEther servers
// Requires `DisableJsonRpcWebApi` to be set to `false` in SoftEther config
package main
...
func banner() {
	...
	fmt.Println("---\tcmd\t---")
	fmt.Println("run\tRun the API & Web Server")
	fmt.Println("runapi\tRun the API only")
}
...
func main() {
	...
	fmt.Printf("running on localhost:%s", sport)
...
func createUser(w http.ResponseWriter, r *http.Request) {
	...
	if strings.Contains(string(body), "Error code 66") {
		fmt.Fprintf(w, fmt.Sprintf("{'error': %s}", string(body)))
		//fmt.Fprintf(w, "error")
		return
	}
...
// viewUser returns user information as JSON Marshal string
